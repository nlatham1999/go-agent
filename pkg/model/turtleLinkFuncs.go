...
func (t *Turtle) descendents(checkForRotated bool, checkForMoving bool, checkForSwivelling bool) *TurtleAgentSet {
	...
	generalBreed := t.parent.directedLinkBreeds[BreedNone]

	outgoing := t.parent.linkedTurtles[t].getLinksOutgoing(generalBreed)
	for outgoing.Count() > 0 {
		...
		if t1 != t && !d.Contains(t1) {
			d.Add(t1)
			nextLinks := t1.parent.linkedTurtles[t1].getLinksOutgoing(generalBreed)
		...
		if t2 != t && !d.Contains(t2) {
			d.Add(t2)
			nextLinks := t2.parent.linkedTurtles[t2].getLinksOutgoing(generalBreed)
			nextLinks.Ask(func(l2 *Link) {
				if d.Contains(l2.end2) && d.Contains(l2.end1) {
					return
				}
